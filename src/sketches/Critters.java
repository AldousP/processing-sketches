 ...
 * Spring Grid
 */
public class Critters extends BaseSketch {
    Spring[] springs;
    int gridX = 25;
    int gridY = 25 ;
    ...
    protected float automatonRadius = .0085f;
    protected float obstacleRadius = .095f;
    int obstacleCount = 10;
    int automatonCount = 10;
    int OBSTACLE_COLOR;
    ...
    public void setup() {
        ...
        for (int i = 0; i < automatonCount; i++) {
            tmp = new PVector(random(-0.5f, 0.5f), random(-0.5f, 0.5f));
            automatons.add(new Automaton(tmp, obstacles, automatons,  1, 1, automatonRadius));
        }
    ...
    protected class Spring {
        ...
        void update(float dampening, float tension, float topSpeed) {
            ...
            for (Automaton a : automatons) {
                float dst = abs(a.position.copy().sub(position).mag());
                if (dst < attractRadius) {
                    height = ((dst) / attractRadius);
                    speed =  dst / 10;
                    rotation = SolMath.getRelativeRotationOfPoint(a.position, position);
                }
            }
